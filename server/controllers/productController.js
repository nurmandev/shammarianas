...
const searchProduct = async (req, res) => {
  try {
    ...
    if (count < 1) {
      ...
      res
        .status(200)
        .json({ status: 200, count, page, limit, products: matchingProduct });
  } catch (error) {
    console.log(error);
    ...
    if (!productExist) {
      ...
      if (dataExist) {
        ...
        const discount_price =
          req.body.discount_price || productExist.discount_price;
...
const downloadProducts = async (req, res) => {
  try {
    ...
    if (!filenames || !filenames.length) {
      return res
        .status(400)
        .json({ status: 400, message: "Please provide valid filenames" });
    }
