...
const getCustomers = async (req, res) => {
  try {
    ...
    if (count < 1) {
      res
        .status(404)
        .json({ status: 404, message: "There is no customers here", data: [] });
    } else
      res
        .status(200)
        .json({ status: 200, count, page, limit, data: customers });
  } catch (error) {
...
const searchCustomer = async (req, res) => {
  try {
    ...
    if (count < 1) {
      ...
      res
        .status(200)
        .json({ status: 200, count, page, limit, data: matchingCustomer });
  } catch (error) {
