  ...
  try {
    ...
    if (user) {
      ...
      if (passwordMatch) {
        ...
        res
          .status(200)
          .json({ status: 200, message: "login success", accessToken, user });
...
const resetPasswordVerify = async (req, res) => {
  try {
    ...
    if (user) {
      ...
      res.status(404).json({ status: 404, message: "customer does not exist" });
  } catch (error) {
...
const getAccessToken = async (req, res) => {
  try {
    ...
    if (!refreshToken) {
      res
        .status(401)
        .json({ status: 401, message: "You must to be connected" });
    } else {
    ...
    res
      .status(400)
      .json({ status: 400, message: "Failed to generate new access token" });
  }
};
