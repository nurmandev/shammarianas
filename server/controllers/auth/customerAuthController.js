...
const customerSignup = async (req, res) => {
  try {
    ...
    if (userExist) {
      res.status(403).json({
        status: 401,
        message: "Can't create account for admin or manager",
      });
    } else {
      if (customerExist) {
        ...
        const mainToken = await token.mainToken(
          customer.id,
          customer.username,
          "customer"
        );
    ...
    // } else
    //   res.status(403).json({ status: 400, message: "Bots are not allowed!" });
  } catch (error) {
...
const customerLogin = async (req, res) => {
  try {
    ...
    if (customer && customer.valid_account) {
      ...
      res.status(401).json({ status: 401, message: "Invalid email address" });
    // } else
    //   res.status(403).json({ status: 400, message: "Bots are not allowed!" });
  } catch (error) {
...
const customerGoogleLogin = async (req, res) => {
  try {
    ...
    if (credential) {
      ...
      res.status(404).json({ status: 403, message: "invalid credentials" });
  } catch (error) {
    res.status(500).json({
      status: 500,
    ...
    if (verificationToken) {
      ...
      if (customer) {
        if (customer.valid_account) {
          res
            .status(400)
            .json({ status: 400, message: "This email is already validated" });
        } else {
...
const resetPasswordVerification = async (req, res) => {
  try {
    ...
    if (customer && customer.valid_account) {
      ...
      // Set message template for the mail verification
      const messageTemplate = await template.resetPassword(
        customer.username,
        mainToken
      );
      ...
      res
        .status(404)
        .json({ status: 404, message: "This account does not exist" });
  } catch (error) {
  ...
  try {
    ...
    if (verificationToken) {
      ...
      res.status(403).json({
        status: 403,
        message:
          "Please verify your email before proceeding with the password reset.",
      });
  } catch (error) {
    res.status(500).json({
      status: 500,
...
const customerLogout = async (req, res) => {
  try {
    ...
    if (!cookieToken) {
      res
        .status(201)
        .json({ status: 201, message: "Customer is not connected" });
    } else {
      ...
      if (!customer) {
        ...
        res.clearCookie("token", {
          httpOnly: true,
          maxAge: 0, // secure: true
        });
        res
          .status(200)
          .json({ status: 200, message: "Customer logout successfully" });
      }
    }
  } catch (error) {
