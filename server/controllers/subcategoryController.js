...
const createSubcategory = async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({ status: 400, message: "Failed to create subcategory" });
  }
};
...
const getSubcategories = async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({ status: 400, message: "Failed to get subcategories" });
  }
};
  ...
  try {
    ...
    if (!subcategory) {
      ...
      if (!subcategoryWithCategory || subcategoryWithCategory.length === 0) {
        res
          .status(500)
          .json({ status: 500, error: "Failed to retrieve category name" });
      } else {
        res
          .status(200)
          .json({ status: 200, subcategories: subcategoryWithCategory });
      }
    }
  } catch (error) {
...
const searchSubcategories = async (req, res) => {
  try {
    ...
    if (count < 1) {
      ...
      res
        .status(200)
        .json({ status: 200, count, page, limit, data: matchingSubcategories });
  } catch (error) {
    console.log(error);
  ...
  try {
    ...
    if (!subcategoryExist) {
      ...
      const dataExist = await Subcategory.findOne({
        $and: [{ subcategory_name }, { _id: { $ne: subcategoryId } }],
      });
      if (dataExist) {
        return res.status(400).json({
          status: 400,
    ...
    res
      .status(400)
      .json({ status: 400, message: "Failed to update subcategory" });
  }
};
...
const deleteSubcategory = async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({ status: 400, message: "Failed to delete subcategory" });
  }
};
