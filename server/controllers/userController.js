  ...
  try {
    ...
    if (userExist) {
      return res.status(400).json({
        status: 400,
        message:
          "User already exists, check the username and the email address",
      });
    } else {
      // Password hashing
      ...
      const messageTemplate = await template.userCredentials(
        username,
        role,
        email,
        password,
...
const getUsers = async (req, res) => {
  try {
    ...
    if (count < 1) {
      ...
      res.status(200).json({ status: 200, count, page, limit, data: users });
  } catch (error) {
...
const searchUser = async (req, res) => {
  try {
    ...
    if (count < 1) {
      ...
      res
        .status(200)
        .json({ status: 200, count, page, limit, data: matchingUser });
  } catch (error) {
  ...
  try {
    ...
    if (!userExist) {
      ...
      const dataExist = await User.findOne({
        $and: [{ $or: [{ email }, { username }] }, { _id: { $ne: userId } }],
      });
      if (dataExist) {
        return res.status(400).json({
          status: 400,
